...
# bar {
#   status_command exec /home/ali/.config/i3status/mybar.sh
# }
...
mycrypto() {
  ...
  echo -n "\"full_text\":\" $(/home/ali/.config/i3status/crypto.py) \","
  echo -n "\"color\":\"#000000\","
...
myip_public() {
  ...
  echo -n "\"full_text\":\" $(/home/ali/.config/i3status/ip.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
disk_usage() {
  ...
  echo -n "\"full_text\":\"  $(/home/ali/.config/i3status/disk.py)%\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "}"
}
...
memory() {
  ...
  echo -n "\"full_text\":\"  $(/home/ali/.config/i3status/memory.py)%\","
...
cpu_usage() {
  ...
  echo -n "\"full_text\":\"  $(/home/ali/.config/i3status/cpu.py)% \","
  echo -n "\"background\":\"#3949AB\","
  common
  echo -n "},"
}
...
meteo() {
  ...
  echo -n "\"full_text\":\" $(/home/ali/.config/i3status/meteo.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
while read line;
do
  # echo $line > /home/ali/gitclones/github/i3/tmp.txt
  # {"name":"id_vpn","button":1,"modifiers":["Mod2"],"x":2982,"y":9,"relative_x":67,"relative_y":9,"width":95,"height":22}

  # VPN click
  if [[ $line == *"name"*"id_vpn"* ]]; then
    alacritty -e /home/ali/.config/i3status/click_vpn.sh &

  # CHECK UPDATES
  elif [[ $line == *"name"*"id_systemupdate"* ]]; then
    alacritty -e /home/ali/.config/i3status/click_checkupdates.sh &
  ...
  # TIME
    ...
    alacritty -e /home/ali/.config/i3status/click_time.sh &

  # METEO
  elif [[ $line == *"name"*"id_meteo"* ]]; then
    xdg-open https://openweathermap.org/city/2913761 > /dev/null &
